...
def scores(roll):
    ...
    a pandas Series or DataFrame that can be concatenated to the scores DataFrame
    """
    ...
    scores = pd.concat([scores, 
                        score_combinations(scores, roll)],
                        axis=0, sort=True).sort_values(['Score'], ascending=False).reset_index()
...
def test():
    ...
    skip = [] # to do all the tests uncomment this line
